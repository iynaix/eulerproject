...
def euler11():
    grid = """08 02 22 97 38 15 00 40 00 75 04 05 07 78 52 12 50 77 91 08
            ...
            """
    grid = [[int(x) for x in line.strip().split()] for
            line in grid.splitlines()]

    def c(x, y):
        """
        returns the number at the given coordinates, 0 if x or y are
        invalid, i.e. less than 0
        """
        if x < 0 or y < 0:
            return 0
        try:
    ...
    for x in range(20):
        for y in range(20):
            ret = max(get_adjacent_sums(x, y), ret)
    return ret
